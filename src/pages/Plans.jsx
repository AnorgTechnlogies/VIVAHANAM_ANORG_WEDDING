...
const Plans = () => {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      // Assuming vendorId can be retrieved or backend relies on token.
    ...
    return (
      <div className="min-h-screen bg-[#f7f6f3] flex flex-col items-center justify-center py-20 px-4">
        <div className="bg-white rounded-3xl p-10 max-w-2xl w-full shadow-2xl text-center">
          ...
          <p className="text-gray-500 mb-8">Thank you for subscribing to the <strong className="text-gray-800">{paymentSuccessDetails.planName}</strong> plan.</p>
          ...
          <div className="bg-gray-50 rounded-2xl p-6 text-left space-y-4 mb-8 border border-gray-100">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="font-semibold text-gray-900 text-sm">{paymentSuccessDetails.amount === 0 ? "100% Discount / Free" : "PayPal"}</span>
            </div>
            {paymentSuccessDetails.startDate && (
              <div className="flex justify-between items-center">
                <span className="text-gray-500">Plan Start</span>
                <span className="font-semibold text-gray-900 text-sm">{new Date(paymentSuccessDetails.startDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
              </div>
            )}
            {paymentSuccessDetails.endDate && (
              <div className="flex justify-between items-center">
                <span className="text-gray-500">Plan Expiry</span>
                <span className="font-semibold text-red-600 text-sm">{new Date(paymentSuccessDetails.endDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
              </div>
            )}
          </div>
  ...
  return (
    ...
    <div className="min-h-screen bg-[#f7f6f3] relative overflow-hidden">
      ...
      {/* Page wrapper */}
      <div
        className={`relative z-10 max-w-6xl mx-auto px-6 py-20 transition-all duration-500 ease-out ${
          visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
        }`}
      >
        {/* Header */}
        <div className="text-center mb-16">
          {/* <span className="inline-block bg-red-100 text-red-700 text-xs font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-5">
            Pricing
          </span> */}
        ...
        {plans.length === 0 ? (
          ...
          <div className="flex flex-wrap justify-center items-start gap-6">
            {plans.map((plan, index) => {
              const isPopular =
                plan.planName.toLowerCase().includes("premium") || index === 1;
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
            <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-300">
              ...
              <p className="text-gray-500 mb-6">
                You are about to subscribe to the <strong className="text-gray-800">{plan?.planName}</strong> plan.
              </p>
              ...
              <div className="flex gap-4">
                ...
                {isFree ? (
                  <button
                    ...
                    disabled={processingPayment}
                    className="flex-1 py-3 px-4 rounded-xl font-semibold text-white shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-70 bg-blue-600 hover:bg-blue-700 shadow-blue-500/30 hover:shadow-blue-500/40"
                  >
                    {processingPayment ? (
                      <>
      ...
      {/* Single keyframe needed for staggered card entrance */}
      <style>{`
        @keyframes slideUp {
